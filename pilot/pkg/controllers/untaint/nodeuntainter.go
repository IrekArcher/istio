...
func NewNodeUntainter(stop <-chan struct{}, kubeClient kubelib.Client, cniNs, sysNs, taintPrefix string, debugger *krt.DebugHandler) *NodeUntainter {
	...
	if !strings.HasSuffix(taintPrefix, "/") && taintPrefix != "" {
		taintPrefix = taintPrefix + "/"
	}
	nt := &NodeUntainter{
		...
		ourNs:       ns,
		taintName:   taintPrefix + DefaultTaintName,
	}
